...
def ransac(data, model, n_minSample, k_maxIteration, t_errorThreshold, d, debug=False, return_all=False):
    ...
    while iterations < k_maxIteration
    {
        ...
        {
            if 满足maybemodel即error < t_errorThreshold
                将点加入alsoinliers
        }
    ...
    while iterations < k_maxIteration:
        maybe_idxs, test_idxs = random_partition(n_minSample, data.shape[0])
...
class LinearLeastSquareModel:
    ...
    def get_error(self, data, model):
        A = np.vstack([data[:, i] for i in self.input_columns]).T  # 第一列Xi-->行Xi
        B = np.vstack([data[:, i] for i in self.output_columns]).T  # 第二列Yi-->行Yi
        B_fit = sp.dot(A, model)  # 计算的y值,B_fit = model.k_maxIteration*A + model.b
...
def test():
    ...
    B_exact = sp.dot(A_exact, perfect_fit)  # y = x * k_maxIteration
